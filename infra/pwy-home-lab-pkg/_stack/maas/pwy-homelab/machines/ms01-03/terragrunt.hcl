...
# Commission ms01-03 in MaaS and deploy Ubuntu noble to it.
#
...
#      providers.maas.config_params["pwy-home-lab-pkg/_stack/maas/pwy-homelab/machines/ms01-03"]
...
locals {
  ...
  #   - chpasswd (if cloud_init_password is set in secrets, using cloud_init_user
  #     from public YAML, defaulting to "ubuntu")
  _ssh_keys = try(local.up.cloud_init_ssh_keys, [])
  _ci_user  = try(local.up.cloud_init_user, "ubuntu")
...
inputs = {
  ...
  # provisioning_ip pins ms01-03 to a fixed IP on the provisioning VLAN (VLAN 12)
  # in MaaS (mode=STATIC on the primary interface). Passed to commission-and-wait.sh
  # as MAAS_STATIC_IP. Set in config_params["...ms01-03"].
