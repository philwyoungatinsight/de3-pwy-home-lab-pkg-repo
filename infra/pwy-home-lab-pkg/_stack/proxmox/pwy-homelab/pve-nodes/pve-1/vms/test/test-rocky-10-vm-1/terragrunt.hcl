...
# The Rocky 10 cloud image must be present on the node before the VM can be created.
dependency "rocky_10_cloud_image" {
  config_path = "../../../isos/rocky-10"
  mock_outputs = {
    file_id   = "local:import/Rocky-10-GenericCloud-Base.latest.x86_64.qcow2"
    node_name = "pve"
  }
...
# Add entries under "pwy-home-lab-pkg/_stack/proxmox/pwy-homelab/pve-nodes/pve-1/vms/test/test-rocky-10-vm-1"
# (or the pve-1 level) to override any of the following:
#
#   vm_name:              test-rocky-10-vm-1 # defaults to the unit directory name
...
#   cloud_init_user:      rocky              # Rocky Linux default user
...
#
# node_name / datastore_vm / network_bridge are inherited from the pve-1
...
locals {
  ...
  # additional_tags accumulates from all ancestor levels (not last-wins like other params).
...
inputs = {
  ...
  # Primary disk (imported from the Rocky Linux 10 cloud image)
  disk_datastore   = local.disk_datastore
  disk_size        = local.disk_size
  disk_interface   = "virtio0"
  disk_import_from = dependency.rocky_10_cloud_image.outputs.file_id
