...
# Rocky 9 ISO must be present before starting the cloned VM (template's CD-ROM references it).
dependency "rocky9_iso" {
  config_path = "../../../isos/rocky-9-installer"
  mock_outputs = {
    file_id   = "local:iso/Rocky-9.7-x86_64-minimal.iso"
    file_name = "Rocky-9.7-x86_64-minimal.iso"
    node_name = "pve"
  }
...
# ---------------------------------------------------------------------------
# VM cloned from the Packer-built template-packer-rocky-9.
...
#   clone_vm_name:      template-packer-rocky-9    # Packer template name (required)
#   vm_name:            test-packer-rocky-9-vm
#   vm_id:              null        # null = auto-assign
#   cloud_init_user:    rocky       # Rocky default user (set in YAML)
#   memory_mb:          8192
#   cpu_cores:          2
#
