...
# The Rocky 9 cloud image must be present on the node before the VM can be created.
dependency "rocky_9_cloud_image" {
  config_path = "../../../isos/rocky-9"
  mock_outputs = {
    file_id   = "local:import/Rocky-9-GenericCloud.latest.x86_64.qcow2"
    node_name = "pve"
  }
...
# Add entries under "pwy-home-lab-pkg/_stack/proxmox/pwy-homelab/pve-nodes/pve-1/vms/test/test-rocky-9-vm-1"
# (or the pve-1 level) to override any of the following:
#
#   vm_name:              test-rocky-9-vm-1  # defaults to the unit directory name
...
inputs = {
  ...
  # Primary disk (imported from the Rocky Linux 9 cloud image)
  disk_datastore   = local.disk_datastore
  disk_size        = local.disk_size
  disk_interface   = "virtio0"
  disk_import_from = dependency.rocky_9_cloud_image.outputs.file_id
