...
# ---------------------------------------------------------------------------
# MaaS rack controller VM — Ubuntu 24.04, tagged role_maas_rack.
# Management IP 10.0.10.13/24 via cloud-init.
# Second NIC on provisioning VLAN 12 — Ansible configures static 10.0.12.2.
# ---------------------------------------------------------------------------
...
locals {
  ...
  # Second NIC on provisioning VLAN so the MaaS rack controller can serve DHCP/PXE.
  # Set network_vlan_id_2: 12 in YAML to enable; null = no second NIC (default).
  network_vlan_id_2 = try(include.root.locals.unit_params.network_vlan_id_2, null)
...
inputs = {
  ...
  # Second NIC on provisioning VLAN — Ansible configures static 10.0.12.2.
  network_bridge_2  = local.network_vlan_id_2 != null ? local.network_bridge : null
  network_vlan_id_2 = local.network_vlan_id_2
