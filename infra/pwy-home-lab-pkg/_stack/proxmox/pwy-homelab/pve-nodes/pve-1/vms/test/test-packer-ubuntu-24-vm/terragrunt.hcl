...
# Ubuntu ISO must be present before starting the cloned VM (template's CD-ROM references it).
dependency "ubuntu_iso" {
  config_path = "../../../isos/ubuntu-24-server"
  mock_outputs = {
...
# Cloud-init vendor-data snippet (installs qemu-guest-agent, ensures it is running)
...
# ---------------------------------------------------------------------------
# VM cloned from the Packer-built template-packer-ubuntu-24.
# The template is built by infra/image-maker-pkg/_tg_scripts/image-maker/build-images during the build stage.
#
# Per-unit overrides via pwy-home-lab-pkg.yaml config_params:
#   clone_vm_name:      template-packer-ubuntu-24  # Packer template name (required)
#   vm_name:            test-packer-ubuntu-24-vm
#   vm_id:              null        # null = auto-assign
#   memory_mb:          8192
#   cpu_cores:          2
#   cpu_sockets:        1
#   cloud_init_user:    ubuntu      # inherited from pve-1
#   cloud_init_ip_address: dhcp     # inherited from pve-1
#
...
locals {
  ...
  clone_vm_name  = include.root.locals.unit_params.clone_vm_name
...
inputs = {
  ...
  # Clone from Packer-built template (no disk_import_from — disk comes from clone)
  clone_vm_name = local.clone_vm_name
  clone_node_name = local.node_name
  clone_datastore = local.disk_datastore
  clone_full    = true
  disk_datastore = null  # disk is provided by the clone; no separate disk block needed

  # Network
  network_bridge      = local.network_bridge
  network_mac_address = local.network_mac_address

  # Cloud-init drive (vendor data only — user/SSH keys from standard cloud-init fields)
